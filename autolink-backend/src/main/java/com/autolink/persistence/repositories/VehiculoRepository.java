...
public interface VehiculoRepository extends JpaRepository<Vehiculo, Integer> {
	...
	@Query("SELECT v FROM Vehiculo v WHERE " +
		       "(:marca IS NULL OR UPPER(v.marca.nombre) = UPPER(:marca)) AND " +
		       ...
		       "(:maxPrecio IS NULL OR v.precio <= :maxPrecio) AND " +
		       "(:maxKm IS NULL OR v.kilometraje <= :maxKm) AND " +
		...
		List<Vehiculo> buscarConFiltros(
		        String marca, String modelo, TipoVehiculo tipo, String color, 
		        Integer minPotencia, Integer maxPrecio, Integer maxKm, Integer plazas, 
		        boolean disponible, boolean aplicarDisp, 
