...
public class JwtUtils {
	...
	public String generateAccessToken(UserDetails userDetails) {
		return JWT.create().withSubject(userDetails.getUsername())
				...
				.withIssuedAt(new Date()).withIssuer(jwtConfig.getIssuer())
	...
	public String extractUsername(String token) {
		return JWT.require(Algorithm.HMAC256(jwtConfig.getSecret())).build().verify(token).getSubject();
	}
	...
	public String generateAccessToken(String token) {
		...
		return JWT.create().withSubject(jwt.getSubject()).withClaim("roles", jwt.getClaim("roles").asList(String.class))
				.withIssuedAt(new Date()).withIssuer(jwtConfig.getIssuer())
				.withExpiresAt(new Date(System.currentTimeMillis() + jwtConfig.getAccessTokenExpires()))
				.sign(Algorithm.HMAC256(jwtConfig.getSecret()));
	}
	...
	public String generateRefreshToken(String token) {
		DecodedJWT jwt = JWT.decode(token);
		return JWT.create().withSubject(jwt.getSubject()).withClaim("roles", jwt.getClaim("roles").asList(String.class))
				.withClaim("type", "refresh").withIssuedAt(new Date()).withIssuer(jwtConfig.getIssuer())
				.withExpiresAt(new Date(System.currentTimeMillis() + jwtConfig.getAccessTokenExpires()))
				.sign(Algorithm.HMAC256(jwtConfig.getSecret()));
	}
