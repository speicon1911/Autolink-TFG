    ...
    <div class="space-y-6 animate-fade-in relative z-0">
      ...
      <div class="grid grid-cols-1 md:grid-cols-[1fr_350px] gap-8">
        ...
        <div class="space-y-6">
            ...
            <form (submit)="createBrand($event)" class="space-y-4">
              <div>
                ...
                <input 
                  ...
                  class="w-full bg-white/10 border border-white/5 text-pitch-black-50 rounded-2xl p-4 focus:ring-2 focus:ring-baltic-blue-500 outline-none transition-all font-bold placeholder:text-baltic-blue-400/30"
                >
              </div>
              <button 
                type="submit"
                [disabled]="!newBrandName().trim() || saving()"
                class="w-full btn-primary text-white font-black py-4 rounded-2xl transition-all shadow-xl shadow-baltic-blue-600/20 active:scale-95 disabled:opacity-50 flex items-center justify-center gap-2"
              >
                @if (saving()) {
          ...
          <div class="bg-baltic-blue-500/10 border border-baltic-blue-500/20 p-6 rounded-3xl">
